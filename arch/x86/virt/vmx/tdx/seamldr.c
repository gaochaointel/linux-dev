...
struct seamldr_params {
	u32	version;
	u32	scenario;
	u64	sigstruct_pa_list[SEAMLDR_MAX_NR_SIG_PAGES];
	u8	reserved[104];
	u64	module_nr_pages;
...
/*
 * Intel TDX module update image. Defined in blob_structure.txt
 * from the "Intel TDX module Binaries Repository"
 *
 * The variable-length payload contains sigstruct pages first,
 * then module pages.
 */
struct tdx_image {
				    ...
				    const u8 *mod, u32 mod_nr_pages)
{
	...
	populate_pa_list(params->sigstruct_pa_list, SEAMLDR_MAX_NR_SIG_PAGES,
			 sig, sig_nr_pages);
...
static int init_seamldr_params(struct seamldr_params *params, const u8 *data, u32 size)
{
	...
	u32 sigstruct_len	= header->sigstruct_nr_pages << PAGE_SHIFT;
	u32 module_len		= header->module_nr_pages << PAGE_SHIFT;
