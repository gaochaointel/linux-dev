...
struct pci_tsm_ops {
	...
	struct_group_tagged(pci_tsm_link_ops, link_ops,
		...
		int (*guest_req)(struct pci_tdi *tdi, void *req_in,
				 size_t in_len, void *req_out, size_t out_len,
				 u64 *tsm_code);
	);
...
DEFINE_FREE(pci_tsm_free_encrypted_resources, struct resource **,
	    if (_T) pci_tsm_free_encrypted_resources(_T))
#else
...
static inline int pci_tsm_guest_req(struct pci_dev *pdev,
				    enum pci_tsm_req_scope scope, void *req_in,
				    size_t in_len, void *req_out,
				    size_t out_len, u64 *tsm_code)
{
