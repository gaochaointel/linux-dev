 ...
 *
 * Returns 0 on success and -error on failure. On success or failure @tsm_code
 * may be populated with a TSM implementation specific result code for the guest
 * to consume.
 *
		      ...
		      size_t out_len, u64 *tsm_code)
{
	...
	if (!tdi)
		return -ENXIO;
	return ops->guest_req(tdi, req_in, in_len, req_out, out_len, tsm_code);
}
...
/**
 * pci_tsm_alloc_encrypted_resources() - mark device MMIO as encrypted in iomem
 * @pdev: device MMIO to mark as encrypted
 * @__res: Caller allocated array of PCI_NUM_RESOURCES pointers
 *
 * For the simple case of all MMIO is encrypted in the RUN state arrange for all
 * BARs to be mapped as encrypted. Returns @__res on success, NULL on failure.
 *
 * This helper, and the related 'set' and 'free' helpers, are amenable to scope
 * based cleanup in the following sequence in the TSM driver ->accept() handler:
 *
 * TODO: Figure out carve outs for MSI-X which are often shared with the VMM
 *
 * ---
 * struct resource *__res[PCI_NUM_RESOURCES];
 *
 * struct resource **res __free(pci_tsm_free_encrypted_resources) =
 *	pci_tsm_alloc_encrypted_resources(pdev, __res);
 *
 * rc = ...TSM driver accept() flow...
 *
 * if (rc)
 *	return rc;
 * pci_tsm_set_encrypted_resources(to_pci_tsm_devsec(pdev->tsm),
 *				   no_free_ptr(res));
 * return 0;
 * ---
 */
struct resource **pci_tsm_alloc_encrypted_resources(struct pci_dev *pdev,
...
EXPORT_SYMBOL_GPL(pci_tsm_alloc_encrypted_resources);

void pci_tsm_set_encrypted_resources(struct pci_tsm_devsec *tsm,
				     struct resource **res)
{
	memcpy(tsm->resource, res, sizeof(tsm->resource));
}
EXPORT_SYMBOL_GPL(pci_tsm_set_encrypted_resources);

void pci_tsm_free_encrypted_resources(struct resource **res)
{
...
EXPORT_SYMBOL_GPL(pci_tsm_free_encrypted_resources);
